...
# Script to plot forest plots of pathogens, diarrhea, and heavyrain 
# with different lags
...
adj = prev_table %>%
  filter(Group == "Adj.", grepl("heavyrain", risk_factor), !grepl("Bruising", Outcome)) %>% #!grepl("percentile", RF),
  ...
  mutate(cutoff = ifelse(grepl("percentile", RF_Type), "90th percentile", "80th percentile"))
...
(diar_plot <- ggplot(adj %>% filter(category=="Diarrhea") %>% mutate(Outcome = "7-day Diarrhea"), aes(x=Outcome, y=PR, group=lag)) +
  ...
  scale_fill_manual(values = c("#024d81",  "#317cb0","#60a6d6", "#955b13","#c77919", "#f99820"), 
                    labels=c("3 week lag", "2 week lag", "1 week lag", "3 week lag", "2 week lag", "1 week lag")) +
...
legend_plot <- ggplot(adj %>% filter(category=="Bacteria"), aes(x=Outcome, y=PR, color=risk_factor, shape = expected_lag_sym, fill = risk_factor, group = risk_factor)) +
  geom_hline(yintercept=1) +
  ...
  coord_flip() +
  scale_colour_manual(values = c("#024d81",  "#317cb0","#60a6d6", "#97ccf0", "#955b13","#c77919", "#f99820", "#faac4c"), 
                      labels=c("3-week lag  (80th percentile cutoff)", "2-week lag", "1-week lag", "0-week lag", "3-week lag  (90th percentile cutoff)", "2-week lag", "1-week lag", "0-week lag")) +
  scale_fill_manual(values = c("#024d81",  "#317cb0","#60a6d6", "#97ccf0", "#955b13","#c77919", "#f99820", "#faac4c"), 
  ...
  scale_y_continuous(trans="log", breaks = c(0.25, 0.5, 1, 1.5, 2,  3, 4), 
                     labels=c(0.25, 0.5, 1, 1.5, 2, 3, 4),
                     limits = c(0.25, 5.25)) +
  guides(color = guide_legend(title = "Heavy Rainfall", reverse = T, order = 1, nrow = 2, byrow=TRUE),
         shape = guide_legend(title = "Expected Lag Period Relevance", reverse = T, order = 2)) +
  theme_bw() +
...
(combined_plot <- grid.arrange(all_disease, legend, heights = c(1.2, .15)))
